...
public class HashTableMovie {
	...
	HashTableMovie(){
		table=new MovieAvlTree[length];
	}
	void insert(Movie current){
		if(table[HashFunction(current.title)]==null)
			table[HashFunction(current.title)]=new MovieAvlTree();
		table[HashFunction(current.title)].insertMovie(current);
	}
	...
	Movie search(String name){
		if(table[HashFunction(name)]!=null)
				return table[HashFunction(name)].searchByTitle(name);
		else
			return null;
