...
public class HashTableActor {
	ActorAvlTree[] table;
	
	HashTableActor(){
		table=new ActorAvlTree[27];
	}
	void insert(Actor current){
		if(table[HashFunction(current.name)]==null)
			table[HashFunction(current.name)]=new ActorAvlTree();
		table[HashFunction(current.name)].insertActor(current);
	}
